...
public class Pila {
    ...
    public void extraeInt() {
        int numeroBuscar = Integer.parseInt(JOptionPane.showInputDialog("Ingrese indice a eliminar"));
        int contador = 1;
        Nodo aux = top;
        // Mientras que aux no sea null iteramos
        while (aux != null) {
            if (numeroBuscar == contador) {
                aux = aux.getAbajo();
                //borramos  aux
                aux = null;
                return;
    ...
    public void buscaLibrosAutor(String autor) {
        ...
        while (aux != null) {
            ...
            if (autorEvaluar.equals(autor.toUpperCase().trim())) {
                JOptionPane.showMessageDialog(null,"Se encontraron "+ encuentros+" del autor los cuales son: " + aux);
                //iteramos y aumentamos la variable de encuentros
                aux = aux.getAbajo();
                encuentros++;
            } else {
                aux = aux.getAbajo();
    ...
    public void minMax() {
        Nodo auxActual = top;
        Nodo auxSiguiente = auxActual.getAbajo();
        Nodo min = null;
        Nodo max = null;

        while (auxSiguiente != null) {
            //preguntamos si auxActual es mayor que auxSiguiente
            if (auxActual.getDato().getAño() > auxSiguiente.getDato().getAño()) {
                //si lo es se lo asignamos a max
                max = auxActual;
                min = auxSiguiente;
            } else {
                //si no lo es hacemos lo contrario
                max = auxSiguiente;
                min = auxActual;
            }
            //avanzamos en la pila
            auxActual = auxActual.getAbajo();
            auxSiguiente = auxSiguiente.getAbajo();
        }

        JOptionPane.showMessageDialog(null, "El libro más nuevo es: " + max + " y el libro más antiguo es " + min);
    }
}
